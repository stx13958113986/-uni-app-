    ...
    <view>
        <view class="my-header">
            ...
            <view class="my-header_logo">
                ...
                <button class="sys_btn" type="primary" @click="auth" v-if="!isLogin">小程序登录授权</button>
            </view>
...
<script>
    ...
    export default {
        ...
        methods: {
            async auth(e) {
                const self = this;
                let loginType = 'weixin'
                // 获得定义在image标签上的logintype属性, 属性取值只能使用小写
                // 授权登录
                const loginResult = await uni.login({
                    provider: loginType,
                })
                const info = await uni.getUserInfo({
                    provider: loginType,
                })
                const data = await self.$api.set_user({
                    code: loginResult[1].code,
                    avatar: info[1].userInfo.avatarUrl,
                    author_name: info[1].userInfo.nickName
                })
                self.$api.get_user({
                    user_id: data.data
                }).then(res => {
